...
public final class RedisKeySpaces {
    ...
    public static void ensureEnableKeyspace(String notifyKeySpaceEventsConfigValue) {
        if (!notifyKeySpaceEventsConfigValue.contains(KEY_SPACE_TYPE)) {
            throw new GovernException(Strings.lenientFormat("ensureEnableKeyspace - Redis config [%s] must has value [%s].", NOTIFY_KEYSPACE_EVENTS, KEY_SPACE_TYPE));
        }
    }
    ...
    public static void ensureNotifyKeyspaceEvents(String notifyKeySpaceEventsConfigValue) {
        ...
        if (missConfigValue.length() > 0) {
            throw new GovernException(Strings.lenientFormat("ensureNotifyKeyspaceEvents - Redis config [%s] must has value [%s].", NOTIFY_KEYSPACE_EVENTS, missConfigValue.toString()));
        }
    }
}
