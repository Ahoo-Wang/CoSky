...
import { type ApiMetadata, type ApiMetadataCapable, type ParameterRequest, api, attribute, autoGeneratedError, del, get, patch, path, post, put, request } from "@ahoo-wang/fetcher-decorator";
...
export class UserApiClient implements ApiMetadataCapable {
    ...
    /**
     * - operationId: `lock`
     * - path: `/v1/users/{username}/lock`
     */
    @put('/v1/users/{username}/lock')
    lock(@path('username') username: string, @request() httpRequest?: ParameterRequest, @attribute() attributes?: Record<string, any>): Promise<boolean> {
        throw autoGeneratedError(username, httpRequest, attributes);
    }
